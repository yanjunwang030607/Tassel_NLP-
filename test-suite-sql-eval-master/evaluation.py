...
if __name__ == "__main__":
    ...
    # 添加参数用于指定黄金查询文件路径
    parser.add_argument('--gold', dest='gold', type=str, help="黄金查询文件的路径")
    # 添加参数用于指定预测查询文件路径
    parser.add_argument('--pred', dest='pred', type=str, help="预测查询文件的路径")
    # 添加参数用于指定包含所有数据库和测试套件的目录路径
    parser.add_argument('--db', dest='db', type=str, help="包含所有数据库和测试套件的目录路径")
    # 添加参数用于指定 tables.json 架构文件路径
    parser.add_argument('--table', dest='table', type=str, help="tables.json 架构文件的路径")
    # 添加参数用于指定评估类型
    parser.add_argument('--etype', dest='etype', type=str, default='exec',
                        help="评估类型,exec 表示测试套件准确性,match 表示原始精确集匹配精度",
                        choices=('all', 'exec', 'match'))
    # 添加参数用于指定是否将黄金值插入预测查询中
    parser.add_argument('--plug_value', default=False, action='store_true',
                        help='是否将黄金值插入预测查询中；适用于模型不预测值的情况。')
    # 添加参数用于指定在评估过程中是否保留 distinct 关键字
    parser.add_argument('--keep_distinct', default=False, action='store_true',
                        help='在评估过程中是否保留 distinct 关键字。默认为否。')
    # 添加参数用于指定是否为每个数据点的测试输入打印进度条
    parser.add_argument('--progress_bar_for_each_datapoint', default=False, action='store_true',
                        help='是否为每个数据点的测试输入打印进度条。')
